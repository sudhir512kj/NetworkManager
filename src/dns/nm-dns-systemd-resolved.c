...
{
	nm_c_list_elem_free_all (&config->configs_head, NULL);
...
prepare_one_interface (NMDnsSystemdResolved *self, InterfaceConfig *ic)
{
	...
	c_list_for_each_entry (elem, &ic->configs_head, lst)
        ...
        const char *hostname)
{
	...
	for (i = 0; i < configs->len; i++) {
		...
		if (!ic) {
			...
			ic->ifindex = ifindex;
			c_list_init (&ic->configs_head);
			g_hash_table_insert (interfaces, GINT_TO_POINTER (ifindex), ic);
		}

		if (!skip) {
			c_list_link_tail (&ic->configs_head,
